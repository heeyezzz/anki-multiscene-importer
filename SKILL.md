...
3. Read [the example-design guide](references/example-design.md), then prepare a JSON file following the schema. Create three to five distinct retrieval conditions, not cosmetic rewrites of one sentence. Each sentence must include one `{{c1::...::填入对应词汇}}` cloze. The clozed form may be an inflection or a phrase. Use short Chinese labels for every populated `Scene` field and standard abbreviations such as `n.` / `v.` / `adj.` in `PartOfSpeech`. Set optional `Theme` to `bauhaus` (default) or `minimal` only.
...
5. **Mandatory content confirmation:** run the importer with `--dry-run`, then present the user with the word, Chinese definition, and every scene, English sentence, translation, and analysis. Do not generate TTS, write to Anki, or use `--confirmed` until the user explicitly approves that content. A request to add a word, or approval to use TTS, is not approval of the generated card content.
6. Run the real import only with `--confirmed` after the explicit content approval. The script rejects unconfirmed imports before generating audio or adding notes.
...
node "$SKILL_DIR/scripts/import-vocabulary.mjs" /absolute/path/to/notes.json --confirmed
```
