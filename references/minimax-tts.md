...
The importer creates six audio files per new note: one for `Word` and one for each unclozed `Sentence1` through `Sentence5`. It checks Anki media for the deterministic filename before each synthesis. A stopped import can be run again: cached files are reused, and only missing audio is charged again. The six audio fields hold raw filenames for click-only playback on the answer side; the question side deliberately renders no audio to avoid revealing the target word. `AudioMediaRefs` holds the matching `[sound:...]` tags but is intentionally not rendered by card templates, preventing automatic playback while preserving Anki media sync and checks.
